...
export function IconPicker({ value, onChange }) {
    ...
    return (_jsxs("div", { className: "relative flex flex-col gap-2", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsxs("button", { type: "button", onClick: () => setOpen(p => !p), className: "flex items-center gap-2 px-3 py-2 rounded-lg border border-border bg-background text-text-secondary text-sm hover:border-primary hover:text-text-primary transition-colors", children: [value ? (_jsxs(_Fragment, { children: [_jsx(Icon, { icon: value, className: "w-5 h-5 text-primary" }), _jsx("span", { className: "text-xs text-text-muted", children: selectedName })] })) : (_jsx("span", { className: "text-text-muted", children: "Sin \u00EDcono \u2014 clic para elegir" })), _jsx("span", { className: "ml-auto text-text-muted text-xs", children: open ? '▲' : '▼' })] }), value && (_jsx("button", { type: "button", onClick: () => onChange(''), className: "text-text-muted hover:text-danger text-xs px-2", children: "\u2715" }))] }), open && (_jsxs("div", { className: "absolute top-full left-0 right-0 z-[200] border border-border rounded-xl bg-surface overflow-hidden shadow-2xl mt-1", children: [_jsx("div", { className: "p-2 border-b border-border", children: _jsx("input", { autoFocus: true, value: search, onChange: e => setSearch(e.target.value), placeholder: "Buscar \u00EDcono... ej: comida, auto, salud, money", className: "input text-sm" }) }), _jsxs("div", { className: "grid grid-cols-8 gap-0.5 p-2 max-h-52 overflow-y-auto", children: [filtered.length === 0 && (_jsxs("div", { className: "col-span-8 text-center text-text-muted text-xs py-6", children: ["Sin resultados para \"", search, "\""] })), filtered.map(icon => {
                                const isSelected = value === icon;
                                return (_jsx("button", { type: "button", title: icon.replace('tabler:', ''), onClick: () => { onChange(icon); setOpen(false); setSearch(''); }, className: `p-1.5 rounded-lg flex items-center justify-center transition-colors
                    ${isSelected
                                        ? 'bg-primary text-white'
                                        : 'text-text-secondary hover:bg-surface hover:text-text-primary'}`, children: _jsx(Icon, { icon: icon, className: "w-5 h-5" }) }, icon));
                            })] }), _jsxs("div", { className: "px-3 py-1.5 border-t border-border text-xs text-text-muted flex justify-between", children: [_jsxs("span", { children: [filtered.length, " \u00EDconos"] }), _jsx("button", { type: "button", onClick: () => setOpen(false), className: "hover:text-text-primary", children: "Cerrar" })] })] }))] }));
}
