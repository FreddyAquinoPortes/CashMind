...
function buildFormatter(moneda, precision, isTotal, mostrarSimbolo) {
    const showDecimals = precision === 'siempre' ||
...
export function useFmt() {
    const { precisionDecimal, mostrarSimbolo, monedaVista, mostrarSaldoOculto } = usePreferenciasStore();
    return (n, isTotal = false) => {
        if (mostrarSaldoOculto)
            return '••••';
...
export function fmtStatic(n, moneda = 'DOP') {
    const num = typeof n === 'string' ? parseFloat(n) : n;
    if (isNaN(num))
        return '—';
    return new Intl.NumberFormat('es-DO', {
